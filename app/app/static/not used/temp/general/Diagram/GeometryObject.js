...
function GeometryObjectOrCollection(translation, rotation, scale, name) {
        ...
        this.lastDrawTransformMatrix = m3.identity();
        this.name = name;
        ...
        this.move = {
                        ...
                        stopTime:      1,
                        time:          0,
                        deceleration: tools.Transform(d2.point(1,1),1,d2.point(1,1)),
...
GeometryObjectOrCollection.prototype.vertexToScreen = function(vertex, canvas) {
        ...
        let transformedVertex = m3.pointTransform(this.lastDrawTransformMatrix, vertex);
        return d2.point((transformedVertex.x+1.0)*canvasWidth/2.0 + canvasRect.left,
...
GeometryObjectOrCollection.prototype.isVelocityZero=function(transform) {
    let threshold = tools.Transform(d2.point(0.001, 0.001), 0.001, d2.point(0.001, 0.001));
...
GeometryObjectOrCollection.prototype.getTransformMatrix = function(transformMatrix, now) {
        ...
        transformMatrix = m3.translate(transformMatrix, this.transform.translation.x, this.transform.translation.y);
        transformMatrix = m3.rotate(transformMatrix, this.transform.rotation);
        transformMatrix = m3.scale(transformMatrix, this.transform.scale.x, this.transform.scale.y);
        this.lastDrawTransformMatrix = m3.copy(transformMatrix);
...
GeometryObjectOrCollection.prototype.transformWithPulse = function(now, transformMatrix) {
    let pulseTransformMatrix = m3.copy(transformMatrix);
    if (this.pulse.pulsing) {
        ...
        pulseTransformMatrix = m3.translate(pulseTransformMatrix, pulseTransform.translation.x, pulseTransform.translation.y);
        pulseTransformMatrix = m3.rotate(pulseTransformMatrix, pulseTransform.rotation);
        pulseTransformMatrix = m3.scale(pulseTransformMatrix, pulseTransform.scale.x, pulseTransform.scale.y);
    ...
    this.lastDrawTransformMatrix = m3.copy(pulseTransformMatrix);
...
GeometryObject.prototype.drawCustom = function(translation, rotation, scale, color) {
        if(this.show) {
            ...
            transformMatrix = m3.translate(transformMatrix, translation.x, translation.y);
            transformMatrix = m3.rotate(transformMatrix, rotation);
            transformMatrix = m3.scale(transformMatrix, scale.x, scale.y);
