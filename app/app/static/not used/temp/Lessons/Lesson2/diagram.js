...
function Lesson2Diagram (lesson, canvas) {
	"use strict";
	Diagram.call(this, lesson, Lesson2Geometry, canvas);

	this.activeObjectList.push(this.geometry.elements['triangle']);
	this.activeObjectList.push(this.geometry._circle._startLine);
	this.activeObjectList.push(this.geometry._circle._stick);
	this.activeObjectList.push(this.geometry._circle._comparisonStick);

	this.touchingOffset = 0;
	this.drawFullCircle = false;
	
	this.annotationValue = document.getElementById("annotation-angle-value");
	this.annotatingValue = false;
	this.portions = 12;
	this.anglePrecision = 1;
...
Lesson2Diagram.prototype = Object.create(Diagram.prototype);

Lesson2Diagram.prototype.draw = function(now) {
	now *= 0.001;
	...
	// this.geometry.setDrawFullCircle(false);
...
// Stick Gestures
Lesson2Diagram.prototype.isTouchingStick = function(point,buffer=1.0) {
	...
	// let rotation = stick.transform.rotation;
	// let zeroRotationPoint = d2.point(point.x, -point.y).rotate(-rotation);
	// // let zeroRotationPoint = geo.rotate(point, -this.geometry._circle._stick.transform.rotation);
	// if ((zeroRotationPoint.x > 0) && (zeroRotationPoint.x < this.geometry._circle.stickLength) &&
	//    (zeroRotationPoint.y >= -stickWidth/2.0*buffer) && (zeroRotationPoint.y <= stickWidth/2.0*buffer)) {
	// 	return true;
	// }
	let stick = this.geometry._circle._stick;
...
Lesson2Diagram.prototype.isIntersectingWithStick = function(p1, p2, buffer=1.0) {
	let stick = this.geometry._circle._stick;
	let circle = this.geometry._circle;
	let stickWidth = this.geometry._circle.stickWidth;
...
Lesson2Diagram.prototype.getStickEnd = function() {
...
Lesson2Diagram.prototype.updateGeometryAngle = function(angle) {
	this.geometry._circle._corner2.transform.rotation = angle;
}
Lesson2Diagram.prototype.setCurrentAngle = function (newAngle, onlyIfMovable = true) {
	let stick = this.geometry._circle._stick;
...
Lesson2Diagram.prototype.updateAnnotation = function() {
		if(this.annotatingValue){
			let newAngle = this.geometry._circle._stick.transform.rotation/Math.PI/2.0 * this.portions;
			this.annotationValue.innerHTML = Math.floor(newAngle*Math.pow(10,this.anglePrecision))/Math.pow(10,this.anglePrecision);
		}
	}

Lesson2Diagram.prototype.getCurrentAngle = function(){
	return this.geometry._circle._stick.current.rotation;
}

Lesson2Diagram.prototype.resetAngle = function() {
...
Lesson2Diagram.prototype.angleFromPoint = function(point) {
...
Lesson2Diagram.prototype.touchDownHandler = function(point) {
...
Lesson2Diagram.prototype.touchMoveHandler = function(previousPoint, point) {
...
Lesson2Diagram.prototype.stopTouches = function() {
...
Lesson2Diagram.prototype.animateFrame = function() {
	this.globals.animateNextFrame();
};
