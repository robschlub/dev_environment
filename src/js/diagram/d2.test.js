...
describe('d2 tests', () => {
  ...
  describe('Lines', () => {
    ...
    describe('Lines can have a length', () => {
      ...
      test('Line from 1, 1 to 11, 11 has length 14.7', () => {
        const l = d2.line(d2.point(1, 1), d2.point(11, 11));
    ...
    describe('Lines can have midpoints', () => {
      test('Line from 0, 0 to 0, 2 has midpoint 0, 1', () => {
        const l = d2.line(d2.point(0, 0), d2.point(0, 2));
        expect(l.midpoint().round()).toEqual(d2.point(0, 1));
      });
      test('Line from 0, 0 to 2, 2 has midpoint 1, 1', () => {
        const l = d2.line(d2.point(0, 0), d2.point(2, 2));
        expect(l.midpoint().round()).toEqual(d2.point(1, 1));
      });
      test('Line from 2, 0 to 4, 0 has midpoint 3, 0', () => {
        const l = d2.line(d2.point(2, 0), d2.point(4, 0));
        expect(l.midpoint().round()).toEqual(d2.point(3, 0));
      });
    });
    describe('Lines can have points within them', () => {
      test('Line <0, 0 2, 0> has point 1, 0 within it', () => {
        const l = d2.line(d2.point(0, 0), d2.point(2, 0));
        const p = d2.point(1, 0);
    ...
    describe('Lines can be compared to other lines', () => {
      ...
      test('Line 1 is not same as Line 2', () => {
        const l1 = d2.line(d2.point(0, 0), d2.point(2, 0));
        const l2 = d2.line(d2.point(0, 0), d2.point(4, 0));
        const result = l1.isEqualTo(l2);
      ...
      test('Line 1 is on the same line as Line 2 - test 2', () => {
        const l1 = d2.line(d2.point(0, 0), d2.point(1, 1));
        const l2 = d2.line(d2.point(2, 2), d2.point(3, 3));
        const result = l1.isOnSameLineAs(l2);
        expect(result).toEqual(true);
      });
      test('Line 1 is on the same line as Line 2 - test 3', () => {
        const l1 = d2.line(d2.point(0, 0), d2.point(1, 1));
        const l2 = d2.line(d2.point(0, 1), d2.point(1, 2));
        const result = l1.isOnSameLineAs(l2);
        expect(result).not.toEqual(true);
      });
    });
    describe('Lines can intersect with other lines', () => {
      ...
      test('Line 0, 0<>2, 0 with 0, 1<>2, 1 has no intersection', () => {
        const l1 = d2.line(d2.point(0, 0), d2.point(2, 0));
        const l2 = d2.line(d2.point(0, 1), d2.point(2, 1));
        const result = l1.intersectsWith(l2);
      ...
      test('Line 0, 0<>2, 0 with 4, 0<>5, 0 has as intersection at 3, 0', () => {
        const l1 = d2.line(d2.point(0, 0), d2.point(2, 0));
        const l2 = d2.line(d2.point(4, 0), d2.point(5, 0));
        const result = l1.intersectsWith(l2);
        expect(result.onLine).toEqual(true);
        expect(result.inLine).toEqual(false);
        expect(result.intersect).toEqual(d2.point(3, 0));
      });
      test('Line 1, 0<>2, 0 with 0, 0<>4, 0 has as intersection at 2.75, 0', () => {
        const l1 = d2.line(d2.point(1, 0), d2.point(2, 0));
        const l2 = d2.line(d2.point(0, 0), d2.point(4, 0));
        const result = l1.intersectsWith(l2);
        expect(result.onLine).toEqual(true);
        expect(result.inLine).toEqual(true);
        expect(result.intersect.round()).toEqual(d2.point(1.75, 0));
      });
      test('Line 0, 0<>2, 0 with 1, 0<>4, 0 has as intersection at 1.5, 0', () => {
        const l1 = d2.line(d2.point(0, 0), d2.point(2, 0));
        const l2 = d2.line(d2.point(1, 0), d2.point(4, 0));
        const result = l1.intersectsWith(l2);
        expect(result.onLine).toEqual(true);
        expect(result.inLine).toEqual(true);
        expect(result.intersect.round()).toEqual(d2.point(1.5, 0));
      });
      test('Line 1, 0<>2, 0 with 1, 0<>4, 0 has as intersection at 1.5, 0', () => {
        ...
        expect(result.onLine).toEqual(true);
        expect(result.inLine).toEqual(true);
        expect(result.intersect).toEqual(d2.point(2, 0));
      });
      test('Line 1, 0<>2, 0 with 2, 0<>3, 0 has as intersection at 2, 0', () => {
        const l1 = d2.line(d2.point(1, 0), d2.point(2, 0));
        const l2 = d2.line(d2.point(2, 0), d2.point(3, 0));
        const result = l1.intersectsWith(l2);
        expect(result.onLine).toEqual(true);
        expect(result.inLine).toEqual(true);
        expect(result.intersect).toEqual(d2.point(2, 0));
      ...
      test('Line 1, 0<>2, 0 with 2, 0<>3, 0 has as intersection at (2, 0) 2', () => {
        const l1 = d2.line(
          d2.point(0.804, 0.04029297190976889),
          d2.point(0.804, 0.05036621488721111),
        );
        const l2 = d2.line(
          d2.point(0.804, 0.05036621488721111),
          d2.point(0.804, 0.06043945786465334),
        );
        const result = l1.intersectsWith(l2);
        expect(result.onLine).toEqual(true);
        expect(result.inLine).toEqual(true);
        expect(result.intersect).toEqual(d2.point(0.804, 0.05036621488721111));
      });
    });
  });
  ...
  describe('Minimum angle difference can be calculated from two angles', () => {
    test('30 - 20 = 10', () => {
      const result = d2.minAngleDiff(30 * Math.PI / 180, 20 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(10 * Math.PI / 180, 8));
    });
    test('20 - 30 = -10', () => {
      const result = d2.minAngleDiff(20 * Math.PI / 180, 30 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(-10 * Math.PI / 180, 8));
    });
    test('10 - -10 = 20', () => {
      const result = d2.minAngleDiff(10 * Math.PI / 180, -10 * Math.PI / 180);
    ...
    test('10 - 350 = 20', () => {
      const result = d2.minAngleDiff(10 * Math.PI / 180, 350 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(20 * Math.PI / 180, 8));
    });
    test('350 - 10 = -20', () => {
      const result = d2.minAngleDiff(350 * Math.PI / 180, 10 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(-20 * Math.PI / 180, 8));
    });
    test('350 - 0 = -10', () => {
      const result = d2.minAngleDiff(350 * Math.PI / 180, 0 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(-10 * Math.PI / 180, 8));
    });
    test('370 - 350 = 20', () => {
      const result = d2.minAngleDiff(370 * Math.PI / 180, 350 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(20 * Math.PI / 180, 8));
    });
    test('350 - 370 = -20', () => {
      const result = d2.minAngleDiff(350 * Math.PI / 180, 370 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(-20 * Math.PI / 180, 8));
    });
    test('170 - 190 = -20', () => {
      const result = d2.minAngleDiff(170 * Math.PI / 180, 190 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(-20 * Math.PI / 180, 8));
    });
    test('190 - 170 = 20', () => {
      const result = d2.minAngleDiff(190 * Math.PI / 180, 170 * Math.PI / 180);
      expect(Math.round(result, 8)).toEqual(Math.round(20 * Math.PI / 180, 8));
    });
  });
  describe('Angles can be normalized to between 0 and 360', () => {
    test('30 -> 30', () => {
      const result = d2.normAngle(30 * Math.PI / 180);
    ...
    test('360 -> 0', () => {
      const result = d2.normAngle(360 * Math.PI / 180);
